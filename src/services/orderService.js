...
function formatOrderAmounts(order) {
  ...
  return {
    ...
    grossSubtotalDisplay: formatUsd(order.grossSubtotal ?? order.subtotal ?? order.total),
    taxDisplay: formatUsd(order.tax ?? 0),
    totalDisplay: formatUsd(order.finalAmount ?? order.total),
    estimatedTotalDisplay: formatUsd(order.finalAmount ?? order.total),
    payableAmountDisplay: formatUsd(order.finalAmount ?? order.total),
